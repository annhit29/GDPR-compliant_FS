...
class Instrument: # idem que InstrumentORM of enforcer.py of twitt folder, but for any class. only give the logger
    ...
    def __call__(self, target):
        if isinstance(target, type):
            ...
            # # NEW: skip attribute overwrites for Fuse-based classes
            # try:
            #     from fuse import Fuse as FuseBase
            #     is_fuse = issubclass(target, FuseBase)
            # except Exception:
            #     is_fuse = False
    ...
    def instrument_cls(self, target):
        ...
        def instr(x):
            if callable(x):
                f = self.logger.pep[(qualname,x.__name__)] #qualname is class name in instrument.py. eg: qualname = C. et qualname = C.D veut dire que la classe C avec la sous-classe D
        ...
        # # Decorate each method independently; do NOT reuse a single wrapper.
        # # Also, don't rely on dir(target); decorate only names present in the PEP mapping.
        # for (cls_name, meth_name), f in self.logger.pep.mapping.items():

        #     if cls_name != qualname:
        #         continue
        #     x = getattr(target, meth_name, None)
        #     if x is None:
        #         continue
        #     instrumented = instr(x)
        #     setattr(target, meth_name, instrumented)
        for expr in dir(target):
            ...
            if not hasattr(target, '_is_decorated'):
                instrumented_x = instr(x) #utilise instr(x) pour modifier l'objet x
    ...
    def decorate_function(self, x, f : Callable[..., list[Event]], target, qualname : str):
        def wrapper(inst, *args, **kwargs):
            print(f"[INSTRUMENT] → entering {target.__qualname__}.{x.__name__} with args={args}, kwargs={kwargs}", flush=True)
            events = f(*args, **kwargs)
            print(f"[INSTRUMENT] → events = {events}", flush=True)
            cau_flag, sup_flag, cau_events, sup_events = self.check_round_trip_time(events, qualname, 1,  target.__qualname__, False, *args, **kwargs)
    
            if cau_flag:
                print(f"[INSTRUMENT] → invoking causation handler for {target.__qualname__}.{x.__name__}", flush=True)
            ...
            if sup_flag:
                print(f"[INSTRUMENT] → invoking suppression handler for {target.__qualname__}.{x.__name__}", flush=True)
            ...
            # print(f"[INSTRUMENT] → just testing if no causation nor suppression handlers invoked for {target.__qualname__}.{x.__name__}", flush=True)
            # return x(inst, *args, **kwargs)
        
            result = x(inst, *args, **kwargs)
            print(f"[INSTRUMENT] ← leaving {target.__qualname__}.{x.__name__}, returned {result}", flush=True)
            return result
        return wrapper

...
class Instrument: # idem que InstrumentORM of enforcer.py of twitt folder, but for any class. only give the logger
    ...
    def __call__(self, target):
        if isinstance(target, type):
            ...
            # skip attribute overwrites for Fuse-based classes
            try:
                from fuse import Fuse as FuseBase
                is_fuse = issubclass(target, FuseBase) # to avoid overwriting __getattribute__ and __setattribute__ of Fuse-based classes
            except Exception:
                is_fuse = False
    ...
    def decorate_function(self, x, f : Callable[..., list[Event]], target, qualname : str):
        def wrapper(inst, *args, **kwargs):
            ...
            # Debug: show all keyword arguments kw passed from Instrument
            if kwargs:
                print("[INSTRUMENT DEBUG] kwargs content:")
                for k, v in kwargs.items():
                    print(f"   {k}: {v}")
            else:
                print("[INSTRUMENT DEBUG] No kwargs passed.")

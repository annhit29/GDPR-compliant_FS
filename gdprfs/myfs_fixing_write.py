...
# run as normal user
# UPPER_DIR  = Path("/home/ann20010929/gdprfs-test/upper")
# MIRROR_DIR = Path("/home/ann20010929/gdprfs-test/mirror")
...
suppression_handlers = {
    ('Use'): none_handler,
    ('Collect'): none_handler,   # ✅ add this line
}
...
pep = PEP(
    # todo: first start with fid: create, delete, etc.
    mapping={
        ('MyFS', 'read'): Functional(
    'Use',
    lambda path, *a, **kw:
        [] if os.path.basename(path).startswith(".goutputstream-")
        ...
        # todo: have a FS that is hidden from the user, only the copy of the files. V
        ('MyFS', 'write'): Functional('Collect', lambda path, *a, **kw: [Event('Collect', path)]),
...
class MyFS(Fuse):
    ...
    #     # actually open the backing file and store its handle
    #     fh = os.open(p, os.O_RDONLY if flags & os.O_RDONLY else os.O_RDWR)
    #     print(f"[DEBUG open] got OS file handle {fh} for {p}", flush=True)

    #     # tell FUSE to remember this handle
    #     self.file = fh   # optional: store somewhere if you want to reuse
    #     return 0         # must return 0 for “success”

    
    # def read(self, path, size, offset, fh=None):
    #     print(f"[DEBUG read] called with path={path}, size={size}, offset={offset}, fh={fh}", flush=True)
    #     if fh is None:
    #         fh = getattr(self, "file", None)
    #     if fh is not None:
    ...
    #             f.seek(offset)
    #             data = f.read(size)
    #     print(f"[READ] path={path}, len={len(data)} bytes")
    #     return data
    ...
    def read(self, path, size, offset, fh=None):
        ...
        if isinstance(size, (bytes, bytearray)):
            print(f"[WARN] Misrouted write() detected — redirecting safely to raw write for {path}")
            data = size
            # Disable instrumentation temporarily
            old_pep = self.logger.pep
            self.logger.pep = None
            try:
                return MyFS.write(self, path, data, offset, fh)
            finally:
                self.logger.pep = old_pep

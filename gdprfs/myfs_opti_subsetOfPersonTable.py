...
def update_file_people_from_llm(path_abs: str, llm_results: list):
    ...
    with Session() as s:
        ...
        for chunk in llm_results:
            ...
            # only compare against LLM-filtered known users
            chunk_known = chunk.get("known_users", [])
            known_map = {
                ku["full_name"].lower(): ku
                for ku in chunk_known
            }
        ...
        for chunk in llm_results:
            ...
            for person_info in chunk["analysis"]["persons"]:
                if person_info["is_known_user"]:
                    continue
                ...
                for ku in chunk_known:
                    full = ku["full_name"].lower()

                    # matching rule: partial match, first-name match, etc
                    if detected_norm in full or full in detected_norm:
                        alerts.append({
                            "alias": detected,
                            "candidate": ku["full_name"],
                            "person_id": ku["user_id"]
                        })

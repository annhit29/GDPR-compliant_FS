...
pep = PEP(
    ...
    mapping={
        ('MyFS', 'read'): Functional('Use', lambda path, *a, **kw: [Event('Use', "14a-123", "marketing")]), #todo: for the username, use the kw to find the file uid; o/w create a script for finding the file uid
        # todo: create my own db with sqlite3
        # todo: have a FS that is hidden from the user, only the copy of the files
        # todo: create a bash script to automate terminal2 then terminal1
...
class MyFS(Fuse):
    ...
    def getattr(self, path): # get file attributes
        ...
        if path == "/":                           # if the path is the root directory
            st.st_mode  = stat.S_IFDIR | 0o777 #0o755 # directory
            st.st_nlink = 2
            st.st_size  = 0
        elif path == self.HELLO_PATH:             # if the path is /hello.txt file
            st.st_mode  = stat.S_IFREG | 0o777 #0o644 # regular file
            st.st_nlink = 1
    ...
    def access(self, path, mode):
        # allow access to root and our one file
        if path in ("/", self.HELLO_PATH):
            return 0
        from errno import ENOENT
        raise OSError(ENOENT, "No such file or directory")

    def open(self, path, flags):
        # FUSE often calls open() before read(); return 0 to allow it
    ...
    def read(self, path, size, offset, fh=None):
        ...
        print(f"[READ] path={path}, size={size}, offset={offset}, returning={chunk}")
        return chunk
...
if __name__ == "__main__":
    ...
    # Apply the decorator logger, then create an object

...
def _extract_odt_paragraphs(abs_path: str):
    """
    Return a list of paragraph strings extracted from an ODT file.
    ...
    try:
        doc = odt_load(abs_path)
    except Exception as e:
        print("[ODT ERROR] Failed to load ODT:", e)
        return []
    ...
    for p in paras:
        # extractText walks the whole subtree (P, spans, etc.)
    ...
    print(f"[ODT DEBUG] Extracted {len(out)} non-empty paragraphs from {abs_path}")
    for i, t in enumerate(out):
        print(f"[ODT DEBUG] para[{i}] = {repr(t)}")
...
#     # For PDF and ODT, we do fine-grained enforcement *inside* read(),
#     # so we must NOT emit full-file Use events here (otherwise PEP may suppress the read).
#     if lower.endswith(".pdf") or lower.endswith(".odt"):
#         return []

#     # For all other files, keep old behavior
...
class MyFS(Fuse):
    ...
    # --------------------------------------------------------------
        # FIX 1: SKIP DB mapping while LibreOffice is still writing ODT
        # --------------------------------------------------------------
        if path.lower().endswith(".odt") and offset != 0:
            print("[ODT] Partial write detected (offset != 0) — skipping DB mapping")
            _sync_to_mirror(path)
            # No LLM, no DB mapping, no events yet
            return len(data)
    ...
    def _get_or_build_redacted_odt(self, path):
        """
        Build (once) and cache a redacted ODT version of the file.
        Paragraphs that trigger suppressed Use events are replaced with 'REDACTED'.
        """
        abspath = _upper(path)

        # Cache
        cache = PDF_CACHE.get(abspath)
        if cache and "redacted_odt_bytes" in cache:
            return cache["redacted_odt_bytes"]

        # 1) Extract paragraphs
        paragraphs = _extract_odt_paragraphs(str(abspath))
        ...
        # 2) Determine base file_id
        fid_base, _ = _get_file_and_user(abspath)
        if not fid_base:
            fid_base = os.path.basename(abspath)

        # 3) Build new ODT and redact paragraphs selectively
        new_doc = OpenDocumentText()

        # 3) Replace BODY of the document completely
        body = new_doc.body
        textnode = body.firstChild  # <office:text>
        ...
        for idx, para in enumerate(paragraphs):
            para_lc = (para or "").lower()
            uids = _uids_from_page_text(para_lc)

            if not uids:
                # # no personal data → keep paragraph
                # p_el = P(text=para)
                # new_doc.text.addElement(p_el)
                # continue
                final_text = para
            else:
            # Build Use events for each uid on "fid_base/para-i"
                para_fid = f"{fid_base}/paragraph-{idx}"
                events = [Event("Use", para_fid, uid) for uid in uids]

                cau, sup, _, _ = logger.log(events, threading.Event(), False)
                final_text = "REDACTED" if sup else para
            
            p_el = P(text=final_text)
            textnode.addElement(p_el)


        # 4) Serialize new ODT into memory
        # from io import BytesIO
        # buf = BytesIO()
        # new_doc.save(buf)
        # redacted_bytes = buf.getvalue()
        # 4) Serialize new ODT into a temporary file (odfpy cannot save to BytesIO)
        import tempfile, os

        with tempfile.NamedTemporaryFile(suffix=".odt", delete=False) as tmp:
            tmp_name = tmp.name

        # Save proper ODT ZIP structure
        ...
        PDF_CACHE[abspath] = {"redacted_odt_bytes": redacted_bytes}
    ...
    def read(self, path, size, offset, fh=None):
        ...
        # =========== CASE: ODT with paragraph-based enforcement ===========
        if str(p).lower().endswith(".odt"):
            print("[ODT] Paragraph-based enforcement for ODT read")

            redacted_bytes = self._get_or_build_redacted_odt(path)
            total = len(redacted_bytes)
            ...
            if offset >= total:
                ...
                chunk = redacted_bytes[offset:end]

            print(f"[ODT READ DEBUG] SLICE [{offset}:{end}] → {len(chunk)} bytes")
            ...
            print(f"[ODT READ DEBUG] size={size}, offset={offset}, total={len(redacted_bytes)}")

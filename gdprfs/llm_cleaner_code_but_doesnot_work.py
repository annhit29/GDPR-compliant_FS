...
import requests
from gdprfs.models import Session
    ...
    with Session() as s:
        ...
        for chunk in llm_results:
            ...
            for person_info in persons:
                ...
                raw_name = person_info["name"].strip()
                tokens = raw_name.split()
                raw_norm = raw_name.lower() # normalized full name by lowercasing for matching
                print(f'{raw_name=}, {tokens=}, {raw_norm=}')

                # Determine FIRST/LAST name properly
                if len(tokens) >= 2:
                    # Multi-token → split into firstname and lastname
                    first = tokens[0]
                    last  = " ".join(tokens[1:])
                else:
                    # Single-token → heuristic: treat as lastname
                    first = ""
                    last  = tokens[0]

                # Case 1: if Known registered user
                if person_info["is_known_user"]:
                    person = s.query(Person).filter_by(id=person_info["user_id"]).first()
                    # person = s.get(Person, person_info["user_id"])
                else:
                    # Case 2: Alias has already been validated in internal plateform UI

                    # First, check if this token is a known alias validated by the internal UI
                    # alias_norm = name.strip().lower()
                    alias_row = (
                        s.query(NameAlias)
                        .filter(func.lower(NameAlias.alias) == raw_norm)#alias_norm)
                        .first()
                    )
                    print(f'{alias_row=}')
                    print(f'{alias_row.person_id if alias_row else None=}')

                    if alias_row:
                        # Human already confirmed: alias → canonical person
                        person = s.get(Person, alias_row.person_id)
                        print(f"[LLM duplicate DS / alias already merged] Already-merged alias '{raw_name}' which is the registered person id={person.id}")
                    else:
                        # Case 3: Unregistered or new detected person
                        ...
                        print(f'{person=}')
                        if not person:
                            # then create new unregistered person
                            person = Person(first_name=first, last_name=last, uid=None, registered=False)
                            s.add(person)
                            s.commit()
                            print(f"[LLM unregistered DS] Added new unregistered user: {first} {last}")

                # 3.1 Add to GLOBAL file->people mapping
                if person not in file_obj.people:
                    file_obj.people.append(person)

        # Do Step 3 for all chunks. So have the full list of persons in DB first.
        # Then Step 4 for all chunks again (but read-only). So that new persons are already in DB.

        # Preload two things:
        # Preload internal user manually-confirmed aliases to avoid spamming alerts
        known_aliases = {
            a.alias.lower()
        ...
        alerts = []  # list of merge alerts to create (step 5)
        seen_alerts = set() # to avoid duplicates in alerts when multiple chunks have same potential alias
        for chunk in llm_results:
            for person_info in chunk["analysis"]["persons"]: # list of {name, is_known_user, user_id, confidence}
                ...
                for (first_registered, last_registered), reg_person in registered_people.items(): # iterate over registered users with the first and last names gotten
                    # -----------------------------
                    ...
                    if len(tokens) >= 2:
                        ...
                        # if first name typo or last name typo
                        # if det_first == first_registered or _is_typo(det_first, first_registered):
                        if _is_typo(det_first, first_registered) or _is_typo(det_last, last_registered):
                            key = (detected_norm, reg_person.id)
                    ...
                    # -----------------------------
                        ...
                        # if _is_typo(word, first_registered):
                        # if the word is a typo of either first or last name:
                        if _is_typo(word, first_registered) or _is_typo(word, last_registered):
                            key = (detected_norm, reg_person.id)
                            if key not in seen_alerts:
                                seen_alerts.add(key)

...
def events_for_rename(old: str, new: str):
    """
    Treat rename(temp → real) as a 'write' Use on the final file.
    Only enforce when we see the gedit pattern: .goutputstream-* → real filename.
    """
    base_old = os.path.basename(old)
    base_new = os.path.basename(new)

    # Only treat gedit-style temp → real rename as a write
    if base_old.startswith(".goutputstream-") and not _is_temp_name(new):
        # Reuse same logic as a normal read on the final file:
        return events_for_read(new)

    # For other renames, we don't enforce anything (no Use event)
    return []
...
pep = PEP(
    mapping={
        ...
        ('MyFS', 'write'): Functional('Use', lambda path, *a, **kw: events_for_read(path)),

        # gedit-style overwrites: rename(.goutputstream-XXX → jd.txt) = "write" Use on jd.txt
        ('MyFS', 'rename'): Functional('Use',   lambda old, new, *a, **kw: events_for_rename(old, new)),
...
class MyFS(Fuse):
    ...
    def _emit_collect_event(self, path: str):
        ...
        try:
            ...
            for uid in uids or ['noone']:
                evt = Event('Use', fid, uid)
            logger.log([evt], threading.Event(), False)

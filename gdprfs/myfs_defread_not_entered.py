...
def none_handler(event_name, event_args, response, *args, **kwargs):
    ...
    """
    print("inside none_handler")
    # mark that this call was suppressed
    os.environ["GDPR_SUPPRESSED"] = "1"
    return None
...
pep = PEP(
    mapping={
        ...
        ('MyFS', 'open'): Functional('Use', lambda path, *a, **kw: events_for_read(path)), # for empty files read
...
class MyFS(Fuse):
    ...
    # def open(self, path, flags):
        ...
        # # if p.exists():
        #     # handle empty files (size == 0): open an empty file is considered a 'read' operation too
        #     try:
        #         if p.stat().st_size == 0:
        #             update_file_mapping_for_upper(str(p.resolve()), context="read")
        #             update_file_metadata(str(p.resolve()), "read")
        #     except Exception as e:
        #         print(f"[DB] Warn: open() metadata update failed for {p}: {e}")
    ...
    def open(self, path, *args, **kwargs):
        ...
        # handle empty files only
        if p.stat().st_size == 0:
            # defer DB update slightly, to allow suppression flag to be set
            def delayed_update():
                if os.environ.get("GDPR_SUPPRESSED") == "1":
                    print(f"[GDPR] Suppressed open({p.name}) → keeping last_action='create'")
                    os.environ["GDPR_SUPPRESSED"] = "0"
                else:
                    update_file_mapping_for_upper(str(p.resolve()), context="read")
                    update_file_metadata(str(p.resolve()), "read")

            # schedule after short delay (gives logger/enforcer thread time to set flag)

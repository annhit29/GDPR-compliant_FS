...
from ..tasks import ingress_request, consent_request
...
def consent(consent_statistics, session_uuid):
    consent_request.delay(
        consent_statistics,
        session_uuid
    )
...
class ConsentView(ValidateServiceOriginsMixin, View):
    def get(self, *args, **kwargs):
        consent(
            self.request.GET.get("statistics", "false") == "true",
...
class ScriptView(ValidateServiceOriginsMixin, View):
    ...
    def post(self, *args, **kwargs):
        ...
        if session:
            return HttpResponse(
                json.dumps({"status": "ok", "session": str(session.uuid), "statistics": session.statistics_consent}), content_type="application/json"
            )
        else:
            return HttpResponse(

...
def info_session(session):
    return str(session.uuid)

@InstrumentORM(logger, 
			   {"Session.service", "Session.identifier", "Session.start_time", "Session.last_seen",
                "Session.user_agent", "Session.browser", "Session.device", "Session.device_type", "Session.os",
                "Session.ip", "Session.asn", "Session.country", "Session.longitude", "Session.latitude", "Session.time_zone",
                "Session.is_bounce"}, 
			   info = info_session, events = {'read', 'write'})
class Session(models.Model):
    ...
    # Consent
    statistics_consent = models.BooleanField(null=True, verbose_name=_("Consent for statistics"))
...
def info_hit(hit):
    try:
        return str(hit.session.uuid)
    except:
        return ""

@InstrumentORM(logger, 
			   {"Hit.initial", "Hit.start_time", "Hit.last_seen", "Hit.heartbeats", "Hit.tracker"
                "Hit.location", "Hit.referrer", "Hit.load_time", "Hit.service"}, 
			   info = info_hit, events = {'read', 'write'})

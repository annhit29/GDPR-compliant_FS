...
class Scenario:
    ...
    request_delete_url = "http://127.0.0.1:8000/delete/"
    ...
    def run(self):
        ...
        elif self.sc == 'request_delete':
            with Task('django.run', 'Measurement for scenario "Delete"'):
                if not self.is_process_running():
                    print("Django server process has terminated")
                    return None
                time_list = []
                ...
                try:
                    ...
                    r = session.post(self.request_delete_url, data=data, allow_redirects=False)
                    t = r.elapsed.total_seconds()

                    return {'sc': self.sc, 'u': self.u, 'n': self.n,  't': t}

                except requests.exceptions.RequestException as e:
                    print(f"An error occurred while requesting deletion: {e}")
...
class Application:
    ...
    def start(self, policy, exe):
        self.app_cmd += [f"--INSTRLIB_EXE={exe}"]
    ...
    def scenarios(self, policy):
        ...
        with Task('minitwit.scenarios', 'Computing scenarios'):
            if policy == 'uninstrumented':
                ...
                self.app_cmd += [f"--INSTRLIB_FORMULA=policies/{policy}.mfotl",
                                 f"--INSTRLIB_SIG=policies/consent.sig"]
            ...
            if policy != 'uninstrumented':
                scenarios.append(
                    Scenario('request_delete', self.app_cmd, self.database, policy)
                )
        return scenarios
